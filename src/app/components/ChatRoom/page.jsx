...
export default function ChatRoom({ selectedUser }) {
  ...
  const handleSendMessage = (message) => {
    ...
    createNewMessage(messageData).then(savedMessage => {
      ...
      socket.emit("message", {
        ...
        sender: userId,
        receiver: receiver._id,
        _id: savedMessage._id,  // Use `_id` from DB
            ...
            if (message && message.sender === selectedUser._id && !seenMessages.has(messageId)) {
              ...
              try {
                const res = await fetch("/api/messages/seen", {
                  ...
                  body: JSON.stringify({ messageId, seenBy: userId }),
                });
                ...
                if (res.ok) {
                  ...
                  socket.emit("message_seen", {
                    ...
                    seenBy: userId,
                    seenAt: updatedMessage.seenAt,  // Ensure this is updated
                  });
  ...
  useEffect(() => {
    ...
    const handleMessageSeen = async (messageId) => {
      try {
        const res = await fetch("/api/messages/seen", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (res.ok) {
          ...
          socket.emit("message_seen", {
            room: roomId,
            messageId: updatedMessage._id,  // Use `_id`
            seenBy: userId,
            seenAt: updatedMessage.seenAt,
          });
    
          setSeenMessages(prev => new Set([...prev, updatedMessage._id])); // Store `_id`
        } else {
          console.error("Failed to mark message as seen");
        }
      } catch (error) {
    ...
    const handleIncomingMessage = (data) => {
      console.log("Message event received:", data);
      setMessages((prev) => {
        const messageExists = prev.some(msg => msg._id === data._id); // Use `_id` instead of `messageId`
